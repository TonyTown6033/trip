    ...
    async def get_comments(self, note_id: str, xsec_token: str, semaphore: asyncio.Semaphore):
        ...
        async with semaphore:
            ...
            try:
                ...
                utils.logger.error(f"[XiaoHongShuCrawler.get_comments] Skip comments for note {note_id}, retry failed: {ex}")
                return
            except DataFetchError as ex:
                utils.logger.error(f"[XiaoHongShuCrawler.get_comments] Skip comments for note {note_id}, data fetch failed: {ex}")
                return
            except Exception as ex:
                utils.logger.error(f"[XiaoHongShuCrawler.get_comments] Skip comments for note {note_id}, unexpected error: {ex}")
                return
